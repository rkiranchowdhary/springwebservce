...
You can call URL's of emmployee service like below.
...
The service will handle `GET` requests for `/hello`. The `GET` request should return a `200 OK` response with JSON in the body that represents a greeting. It should look something like this:
